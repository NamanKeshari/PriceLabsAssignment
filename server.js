...
const ApiCall = async () => {
  ...
  const pageSize = parseInt(args[2]);
  const address = args.slice(3, args.length).join(" ");
  console.log({ pageSize, address });

  const { data } = await axios.post("https://www.vrbo.com/serp/g", {
...
async function main() {
  ...
  console.log({ listings: resData?.results?.listings });
  const listings = resData?.results?.listings;
  ...
  const filteredListings = listings?.filter((listing) => {
    console.log({
      diff: moment(listing?.rateSummary?.beginDate).fromNow(),
      // diff: moment("2022-05-26").fromNow(),
      date,
  ...
  const manipulatedListings = filteredListings?.map((listing) => {
    let temp = [listing?.listingId, listing?.propertyMetadata?.headline];
    let a = listing?.rateSummary?.beginDate;
    let b = listing?.rateSummary?.endDate;

    if (!flag && listing?.rateSummary?.rentNights) {
      for (var m = moment(a); m.diff(b, "days") <= 0; m.add(1, "days")) {
        JSONasPOJO?.rows[0]?.push(m.format("DD/MM/YY"));
      }
      flag = true;
    }

    listing?.rateSummary?.rentNights?.map((rent) => {
      temp.push(rent);
    });

    JSONasPOJO?.rows.push(temp);

    return temp;
    // return {
    //   listingId: listing?.listingId,
    //   listingName: listing?.propertyMetadata?.headline,
    //   rentNights: listing?.rateSummary?.rentNights,
    // };
  });

  console.log({ manipulatedListings, final: JSONasPOJO?.rows });

  let csv = JSONasPOJO.rows.join("\n");

  fs.writeFile("name1.csv", csv, "utf8", function (err) {
    if (err) {
      console.log(
